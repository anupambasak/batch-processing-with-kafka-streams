...
@Slf4j
public class ProtobufDataProducerControllerWithApiCurio {

    private static final String INPUT_TOPIC = "protoMessageTopicV2";

    @Autowired
    KafkaTemplate<String, ProtoBufRecord> protoBufMessageKafkaTemplateWithApicurio;

    @GetMapping("/producev2/protobuf/{producerId}/{count}")
    ...
    private void sendData(String producerId, int count) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            ProtoBufRecord pbr = ProtoBufRecord.newBuilder()
                    .setData(dataRecord)
                    .build();
            protoBufMessageKafkaTemplateWithApicurio.send(INPUT_TOPIC, producerId, pbr);
        }
    }
    ...
    private void sendMetaData(String producerId, int count) {
        ...
        ProtoBufRecord pbr = ProtoBufRecord.newBuilder()
                ...
                .build();
        protoBufMessageKafkaTemplateWithApicurio.send(INPUT_TOPIC, producerId, pbr); // Send with producerId as key and header
